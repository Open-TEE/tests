...
def egcd(a, b):
    ...
    while g1:
        ...
        v, v1 = v1, v - q * v1
        g, g1 = g1, g - q * g1
    return u, v, g #TODO WTF

def convert_integer_to_octet(value, bigint):
    octetstring = hex(abs(value))[2:]
    sign = 1 if value < 0 else 0
    tmp_cstring = ffi.new("uint8_t[]", len(octetstring))
    
    for i in xrange(0, len(octetstring)):
        tmp_cstring[i] = ord(octetstring[i])
...
def convert_octet_to_integer(bigint):
    ...
    tmp_str = ffi.string(tmp_cstring)
    ...
    return sign * bytes_to_long(tmp_str.decode('hex'))
...
class BasicArithmetic(unittest.TestCase):
    ...
    def test_arithmetic(self):
        for i in xrange(0, 1000):
            ...
            bits1 = random.randint(1, 9000)
            bits2 = random.randint(1, 9000)
